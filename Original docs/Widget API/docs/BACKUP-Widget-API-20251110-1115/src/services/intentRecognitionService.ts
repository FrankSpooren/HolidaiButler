...
export class IntentRecognitionService {
  ...
  async recognizeIntent(query: string, conversationContext?: any): Promise<IntentRecognitionResult> {
    try {
      logger.info(`🧠 INTENT RECOGNITION: Analyzing query: "${query}"`);
      
      // Create a comprehensive prompt for Mistral to analyze intent
      const intentPrompt = this.createIntentPrompt(query, conversationContext);
      
      // Use Mistral's chat completion for intent analysis
      ...
      logger.info(`🎯 INTENT DETECTED: ${result.primaryIntent} (confidence: ${result.confidence})`);
      logger.debug(`📊 Intent context:`, result.intentContext);
      
      return result;
      
    } catch (error) {
      logger.error('Intent recognition failed, using fallback:', error);
      return this.fallbackIntentRecognition(query);
    }
  }

  private createIntentPrompt(query: string, conversationContext?: any): string {
    let contextInfo = '';
    ...
    if (conversationContext?.lastResults?.length > 0) {
      const poiNames = conversationContext.lastResults
        ...
        .map((poi: any, index: number) => `${index + 1}. ${poi.title} (${poi.category || 'POI'})`)
        .join('\n');
...
Respond in JSON format:
{
  ...
  private async analyzeWithMistral(prompt: string): Promise<any> {
    let response: string = '';
    try {
      // Use Mistral's chat completion for intent analysis
      ...
      // Debug: Log the raw Mistral response
      logger.info('🔍 MISTRAL RAW RESPONSE:', response);
      ...
      logger.info('🔍 MISTRAL PARSED RESPONSE:', parsed);
      
      return parsed;
    } catch (error) {
      logger.error('Mistral intent analysis failed:', error);
      if (response) {
        logger.error('🔍 MISTRAL RESPONSE THAT FAILED:', response);
      }
  ...
  private parseIntentResult(analysis: any, query: string): IntentRecognitionResult {
    return {
  ...
  private fallbackIntentRecognition(query: string): IntentRecognitionResult {
    logger.info('Using fallback intent recognition');
  ...
  private analyzeBasicContext(query: string): IntentContext {
    const queryLower = query.toLowerCase();
  ...
  private generateBasicActions(primaryIntent: string): string[] {
