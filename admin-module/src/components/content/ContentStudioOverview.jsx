...
function TopContentItem({ item, rank }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1.5, borderBottom: '1px solid', borderColor: 'divider', '&:last-child': { border: 'none' } }}>
      <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: 'text.disabled', width: 20, textAlign: 'center' }}>
        {rank}
      </Typography>
      ...
      <Box sx={{ textAlign: 'right' }}>
        <Typography variant="caption" sx={{ fontSize: '0.7rem', color: 'text.secondary' }}>
          {parseInt(item.total_reach).toLocaleString()} bereik
        </Typography>
        <Typography variant="caption" sx={{ fontSize: '0.65rem', color: 'text.disabled', display: 'block' }}>
          {parseInt(item.total_engagement).toLocaleString()} engagement
        </Typography>
      </Box>
    </Box>
  );
...
export default function ContentStudioOverview({ onNavigateTab }) {
  ...
  useEffect(() => {
    async function fetchOverview() {
      try {
        setLoading(true);
        const { data: res } = await client.get('/content/studio/overview');
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard
            ...
            iconColor={tokens.brand.teal}
            label="Vandaag"
            value={loading ? '-' : data?.today?.scheduled || 0}
            subtext={loading ? '' : `${data?.today?.published || 0} gepubliceerd`}
            action="Kalender"
            onAction={() => onNavigateTab && onNavigateTab(3)}
            loading={loading}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard
            ...
            label="Deze week"
            value={loading ? '-' : data?.thisWeek?.scheduled || 0}
            subtext={loading ? '' : `${data?.thisWeek?.published || 0} gepubliceerd`}
            action="Bekijk"
            onAction={() => onNavigateTab && onNavigateTab(2)}
            loading={loading}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard
            icon={EventBusyIcon}
            iconColor={data?.gaps?.count > 3 ? tokens.semantic.warning : tokens.semantic.success}
            label="Open gaten"
            value={loading ? '-' : data?.gaps?.count || 0}
            subtext={loading ? '' : 'werkdagen zonder content'}
            action="Kalender"
            onAction={() => onNavigateTab && onNavigateTab(3)}
            loading={loading}
          />
        ...
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard
            ...
            iconColor={tokens.brand.teal}
            label="AI suggesties"
            value={loading ? '-' : data?.suggestions?.pending || 0}
            subtext={loading ? '' : 'nieuwe suggesties'}
            action="Bekijk suggesties"
            onAction={() => onNavigateTab && onNavigateTab(1)}
            loading={loading}
          />
        </Grid>
      </Grid>
          ...
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ p: 2.5 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                <WarningAmberIcon sx={{ fontSize: 18, color: tokens.semantic.warning }} />
                Aandachtspunten
              </Typography>
              {loading ? (
                <Box>
                  <Skeleton height={32} sx={{ mb: 0.5 }} />
                  <Skeleton height={32} sx={{ mb: 0.5 }} />
                  <Skeleton height={32} />
                </Box>
              ) : hasAttention ? (
                ...
                <Box sx={{ py: 2, textAlign: 'center' }}>
                  <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.8rem' }}>
                    Alles op orde — geen aandachtspunten
                  </Typography>
                </Box>
              )}
            </CardContent>
              ...
              <CardContent sx={{ p: 2.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, display: 'flex', alignItems: 'center', gap: 1 }}>
                    <TrendingUpIcon sx={{ fontSize: 18, color: tokens.brand.teal }} />
                    Top content (30 dagen)
                  </Typography>
                  <Button size="small" onClick={() => onNavigateTab && onNavigateTab(2)}
                ...
                {data.topContent.map((item, i) => (
                  <TopContentItem key={item.id} item={item} rank={i + 1} />
                ))}
              </CardContent>
            </Card>
          </Grid>
      ...
      {/* Empty state for new destinations */}
      {!loading && !data?.thisWeek?.scheduled && !data?.suggestions?.pending && !hasTopContent && (
        <Card sx={{ mt: 2, textAlign: 'center', py: 4 }}>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Start met het genereren van suggesties of maak een nieuw concept.
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
              <Button variant="contained" onClick={() => onNavigateTab && onNavigateTab(1)}
                sx={{ textTransform: 'none', bgcolor: tokens.brand.teal, '&:hover': { bgcolor: '#029e7d' } }}
              >
                Genereer suggesties
              </Button>
              <Button variant="outlined" onClick={() => onNavigateTab && onNavigateTab(2)}
                sx={{ textTransform: 'none', borderColor: tokens.brand.teal, color: tokens.brand.teal }}
              >
                Nieuw concept
              </Button>
            </Box>
