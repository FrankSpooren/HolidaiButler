...
| **Command v16.0** | **Mobiele Homepage Quality — 14 Punten + Extra's** | **21-03** | **503 API fix (adminPortal.js multi-line string syntax error). Punt 10: Onboarding buttons conform template (rounded-xl, Inter font, subtielere kleuren). Punt 11: Map category filter IDs fix (Beach,Nature → beaches). Punt 12: Profiel → login na onboarding compleet. Punt 13+14: bevestigd werkend. Extra: Leaflet z-index isolation (map overlap drawers fix). Extra: ProgramCard time-of-day dynamisch (ochtend/middag/avond categorieën + tijdslots). 24h klok fix (nooit boven 23:30). Onboarding dismissed → sessionStorage (verschijnt bij elk nieuw bezoek). CTA "Zelf programma samenstellen" → itinerary wizard direct geactiveerd. 1 platform-core + 6 hb-websites bestanden. 8/8 verificatie PASS.** |
...
|--------|-------|-------------|
| **4.34.0** | **2026-04-03** | **Content Studio Multi-Source Image Integratie**. Pexels API client (`pexelsClient.js`): searchPexels() met landscape orientation, genormaliseerde photo objects. Flickr API client (`flickrClient.js`): searchFlickr() met commercial-use license filter (CC BY/CC BY-SA/CC0/PDM), Flickr URL builder. imageSelector.js uitgebreid: tier 4a Unsplash (relevance 0.3) → 4b Pexels (0.25) → 4c Flickr (0.2) cascading fallback. 2 nieuwe admin endpoints: POST /content/images/pexels + POST /content/images/flickr (248 endpoints totaal). contentService.js: searchPexels() + searchFlickr() frontend API methods. Environment variables: PEXELS_API_KEY + FLICKR_API_KEY + FLICKR_API_SECRET op Hetzner. Verified: Pexels 200 OK (2 results), Flickr 200 OK (2 results, CC-licensed). Admin 3x deployed. CLAUDE.md v4.34.0. MS v7.94. |
...
| **4.17.0** | **2026-03-23** | **Merk Profiel Completie + Social Accounts + Destination Management**. Opdrachten 7-10 Merk Profiel afgerond (i18n 4 talen, Knowledge Base context). Facebook/Instagram Meta connect endpoint (System User token, Graph API validatie, AES-256 encrypted storage, verificatielink). Social accounts kanaal-filtering (onboarding sync, "extra kanalen" toggle). Destination bewerken: modules & social kanalen wijzigen via Settings (PUT /destinations/:id/feature-flags). Fixes: branding RBAC destination_admin, content generatie hooks violation, suggesties in destination-taal + brand context, content_goals in AI prompt, media upload 50 bestanden/5min, social_accounts kolomnaam fix, website-analyse in destination-taal + "Overnemen in profiel", Tone of Voice inline velden, destination scoping alle pagina's. 237 endpoints. adminPortal.js v3.36.0. MS v7.77. |
