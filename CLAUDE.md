...
|--------|-------|-------------|
| **4.3.0** | **2026-03-15** | **Fase D: Content Intelligence — Analytics Dashboard + Feedback Loop + Swat.io**. BLOK D.0: Content Analyse Dashboard — 3 nieuwe analytics endpoints (overview+groei, per-item sort/filter, platform CTR+eng.rate). ContentAnalyseTab.jsx (3 sub-tabs: Overzicht/Per Item/Platformen, LineChart, PieChart, BarChart, groei-chips). Performance tab vervangen. BLOK D.1: Feedback Loop (Optie B) — feedbackLoop.js wekelijks keyword-performance correlatie, relevance_score boost/penalize. BullMQ content-feedback-loop (zo 04:00). BLOK D.2: Swat.io rapport: nu NIET switchen. 3 endpoints (188 totaal). 1 job (60 totaal). adminPortal.js v3.28.0. MS v7.65. |
...
| **3.96.0** | **2026-03-10** | **Command v14.0 DEEL A: 5 Resterende Fixes — Customer Portal Kwaliteit**. Footer navigation data-driven + ButtonRenderer + POI detail image layout + slide-in drawer + filter modals. 7 nieuwe + 11 gewijzigde bestanden. 10/10 Hetzner PASS. MS v7.58. |
...
|----------|---------|--------|
| Master Strategie | `docs/strategy/HolidaiButler_Master_Strategie.md` | 7.65 |
