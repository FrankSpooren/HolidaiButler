...
│   └── src/
│       ├── routes/ (holibot.js, ticketing.js, reservations.js, adminPortal.js v3.31.0)
│       ├── services/
...
|--------|-------|-------------|
| **4.8.0** | **2026-03-16** | **Content Studio v5.0 Remediatie — 8 Blokken**. BLOK 3 (P0): repurposeContent() herschreven — AI genereert NIEUWE platform-specifieke versies (niet copy-paste), retry bij char limit overschrijding, Repurpose button + dialog. BLOK 4 (P0): Publish workflow na approval — "Nu Publiceren" / "Inplannen" dialog met best time suggesties. BLOK 2 (P0): Image management — attach/detach endpoints, ContentImageSection (POI suggesties + Unsplash), thumbnail grid. BLOK 1 (P1): SEO v2.0 bevestigd content-type-aware. BLOK 8 (P1): AI model upgrade naar mistral-medium-latest, DeepL translator aangemaakt (wacht op API key), translationService v2.0 DeepL-first + Mistral fallback. BLOK 5 (P1): Social Accounts tab in Content Studio. BLOK 6 (P1): BestTimeToPost component + publish dialog chips. BLOK 7 (P2): ApprovalTimeline component (stap-indicator + log). 2 nieuwe endpoints (210 totaal). 1 nieuw bestand (deeplTranslator.js). adminPortal.js v3.31.0. MS v7.68. |
| **4.7.0** | **2026-03-15** | **Content Module Waves 5+6: Enterprise Workflow + Platform Completion**. Wave 5: approval logging, revisions (auto-snapshot), team comments, content pillars, best-time-to-post, UTM tracking, hashtag engine, bulk operations. Wave 6: X API v2 + Pinterest API v5 clients, 14 content templates, publish retry, brand score, SocialAccountsCards UI. 4 DB tabellen. 19 endpoints (210 totaal). 2 jobs (62 totaal). adminPortal.js v3.31.0. MS v7.67. |
...
| **4.0.0** | **2026-03-14** | **Fase B: Content Engine — AI Content Generatie Motor**. De Redacteur (#23) + De SEO Meester (#24). Content Suggesties + Generator UI. 10 endpoints (168 totaal). 24 agents, 56 jobs. MS v7.62. |
