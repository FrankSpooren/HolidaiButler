...
### Backend-topologie & Cascade-methode (bevestigd 24-06-2026)

**Gedeelde Platform-Core API**: er draait **één** backend-proces — `holidaibutler-api` op **127.0.0.1:3001** (PM2, checkout `/var/www/api.holidaibutler.com/platform-core`, branch `dev`). **Alle** admin-surfaces (`admin.dev` / `admin.test` / prod `admin.holidaibutler.com`) én `api.holidaibutler.com` proxyen `/api/v1` → 3001. **Gevolg**: backend-code is live voor álle omgevingen zodra 3001 (her)start is; er is **géén aparte test-/prod-API** om te deployen.

**Admin-frontend is wél per omgeving gescheiden**: aparte docroots `/var/www/admin.dev.holidaibutler.com`, `/var/www/admin.test.holidaibutler.com`, `/var/www/admin.holidaibutler.com` (prod). De **zichtbare prod-wijziging bij een cascade is dus de admin-bundle-deploy** naar de prod-docroot: `npm run build` → backup docroot → `rm -rf assets` → `cp -a dist/.`.

**CI auto-deploy staat UIT** (`deploy-admin-module.yml`: "AUTO-DEPLOY DISABLED 06-05-2026, Server is SSOT", `on: workflow_dispatch`). Push naar een branch triggert **geen** rsync. **Cascade dev→test→main** = git-merge in een **tijdelijke `git worktree`** (zodat de draaiende API-worktree op dev ongemoeid blijft) + push + handmatige bundle-deploy per docroot met backup. Curl-verificatie per env via `--resolve <host>:443:127.0.0.1` (strikte SNI; een `Host:`-header op `127.0.0.1` geeft anders HTTP 421).

### Open Punten (per 25-06-2026)

- **[OPGELOST] REJECTED-FSM** → gefixt in A4 (`f78075e`, v5.13.0): rejected ruimt `scheduled_at` op (beide schrijfpaden), kalender filtert rejected/archived, resubmit via `rejected→pending_review`. Record 303 opgeschoond.
- **[BLIJFT] FSM-achterdeur (RC2)**: `PATCH /content/items/:id` staat **rauwe** `approval_status`-mutatie toe en omzeilt de FSM-gateway (`transitionStatus`) — de UI-"Afwijzen" loopt hierlangs. **T16 gebruikt dit rauwe save-pad bewust** voor de body-persist. Aparte opruiming: statusovergangen uitsluitend via de FSM; `approval_status` uit de generieke PATCH-`allowedFields`.
- **[NIEUW] T9 — SEO-meta niet per taal**: `seo_meta_title`/`seo_meta_description`/`seo_slug` zijn **SINGLE** kolommen (niet per taal) **en** worden niet meevertaald (het translate-endpoint vertaalt alleen `body`). **Datamodelkeuze nodig** (per-taal SEO-kolommen of JSON) vóór een fix.
- **[BLIJFT] FSM-drift-risico**: dubbele bron — `approvalStateMachine.js` (`TRANSITIONS`) + mirror `contentWorkflowMachine.js` (`DEFAULT_TRANSITIONS`). Helper `allowedTransitions()` (`approvalStateMachine.js:155`) leest de **niet-tenant DEFAULT**-map. De-dup als losse opschoning.
- **[NIEUW] Editor-edit-verlies (RC4-restant, breder dan T16)**: de tab-sync-`useEffect` in `ConceptDialog.jsx` reset `editBody` + `dirty=false` bij elke `langTab`-wissel, terwijl de autosave 10s gedebounced is → een **onopgeslagen handmatige edit** (binnen die 10s) verdwijnt bij een tabwissel. T16 dichtte dit voor het alternatief (directe persist); de generieke edit blijft kwetsbaar.
- **[UPDATE] RC4-status**: **T5/T8/T16 opgelost** (v5.13.0); **T9 open** (datamodel).
- **`feature/media-library-wip` (`6725792`)**: Frank's media-WIP (imageSelector/media*), bewust op een aparte branch, **NIET** in dev/test/main gemerged.
...
|--------|-------|-------------|
| **5.13.0** | **2026-06-25** | **Fase A vervolg — RC2/RC4 Content Studio fixes (A4+A2+T8+T16), gecascadeerd dev→test→main**. **A4 (rejected-FSM, `f78075e`)**: afwijzen ruimt nu `scheduled_at` op via **beide** schrijfpaden — `transitionStatus` (nieuwe `rejected`-tak) én de **rauwe PATCH** `/content/items/:id` (de UI-"Afwijzen" omzeilt de FSM-gateway, = hoe item 303 ontstond); kalender-WHERE → `NOT IN ('deleted','rejected','archived')`; **resubmit** afgewezen item via bestaande `rejected→pending_review` (Optie C, **geen** nieuwe FSM-edge; directe `rejected→approved` blijft 409 via FSM); record 303 opgeschoond. **A2/T5 (eerlijke taal-UI, `97df723`)**: lege taal-tab toont **"nog niet vertaald"** (dashed/gedempt) + eerlijke teller **"X van N talen ingevuld"** i.p.v. kale config-claim; opt-in **"Vertaal naar alle talen"** (frontend-lus over bestaand single-target endpoint, **geen** auto-bulk, geen SEO-meta); twee gedupliceerde Vertalingen-blokken → `renderTranslationPanel()`. **T8 (sanitizer-dekking, `c1f5b67`)**: blog-tak strdt nu **kale/HTML-omwikkelde/trailing** code-fences (de sluit-``` matchte voorheen niet als er `---` ná kwam) — bare ```-regel in HTML-blog is altijd artefact; non-blog-tak ongemoeid; repurpose-sanitize gebruikt **werkelijke content_type** (`website→blog`) i.p.v. hardcoded `'social_post'`; 6 items (143/144/145/189/195/201) opgeschoond. **T16 (alternatief-persist, `1edd8e7`)**: "Gebruik alternatief" persisteert nu **direct** via `handleSaveBody` (terugval A) → overleeft tab-/taalwissel + sluiten (de dirty-flow deed dat niet: tab-sync reset `editBody`+`dirty`, autosave 10s gedebounced); geen auto-vertaling. **Promotie**: dev `1edd8e7` → test `fc02fd1` → main `e5b222a` via worktree-merge + per-docroot bundle-deploy met backup; backend gedeeld op 3001 (A4+T8 al live). Prod-bewijs a–e groen. `feature/media-library-wip` (`6725792`) ongemoeid. |
| **5.12.0** | **2026-06-24** | **Fase A — Content Studio scheduling-correctheid (C2/C1) + UX-polish, gecascadeerd dev→test→main**. **C2 (kalender read-path)**: `GET /content/calendar` toont nu **alleen gedateerde items** (`scheduled_at`/`published_at` binnen venster; `ORDER BY COALESCE(scheduled_at, published_at)`); approved-maar-ongeplande items komen apart terug als `readyToSchedule`-array (niet langer op `created_at` ge-bucket → leek "vandaag"). **C1 (approval-gating + zichtbare 409)**: nieuw veld `can_schedule` per item op `GET /content/concepts/:id`, berekend via de **autoritatieve per-tenant FSM** (`workflowConfigService.getTransitions` + `canTransitionXState`); frontend gate't "Inplannen" op die vlag (géén gedupliceerde transitieregels) en toont een `InvalidTransitionError` (409 → `errors.invalid_transition`) als gekleurde Alert top-center i.p.v. verstopt achter de modal. **C1-POLISH**: `WorkflowProgressIndicator` 3-toestanden (voltooid=teal-gevuld+wit vinkje / actief=teal-gevuld+ring+bold / aankomend=grijs outline) via nieuw thema-token **`tokens.brand.tealDark` (#017A60, wit-op-vulling 5,31:1 WCAG-AA)**; kalender "Klaar om in te plannen"-strook met count-badge + micro-hint (`contentStudio.calendar.readyToScheduleHint`); **publicatiefout gehumaniseerd** — publish-now/republish endpoints → `sendApiError` (forwardt `INVALID_STATE_FOR_PUBLISH` 409 i.p.v. ruwe `PUBLISH_ERROR` 500-string), frontend `formatApiError` → `errors.invalid_state_for_publish` (nl/en). **5 commits**: `ded4239` (C2) → `344771f` (C1) → `778198a` (polish) → `a7a0e1f` (stepper-teal) → `918c578` (tealDark AA); gepromoot dev→test (`3413d36`)→main (`2f08e28`) via tijdelijke-worktree-merge + per-docroot admin-bundle-deploy met backup. Backend draait gedeeld op 3001 (live voor alle admin-surfaces). FSM-transities ongewijzigd (**REJECTED-FSM = open punt**). `feature/media-library-wip` (`6725792`) ongemoeid. |
| **5.11.4** | **2026-06-17** | **Secret Scan CI v3.0 + DNS-afronding + monitoring-opruiming (Laag 4 vervolgdag)**. **Secret Scan workflow faalde** (run #22, schedule op main): trufflehog "exit code 1" bleek GEEN secret-vondst maar een bug in de `trufflesecurity/trufflehog@main` action (ongepind; base/head-diff → base==head op schedule-events). Bewezen via directe CLI-scan over volledige historie: 0 verified/unverified, exit 0; CI fast-fail in 10s. **Fix (consistent met de bestaande gitleaks-CLI-aanpak)**: trufflehog action → gepinde CLI v3.95.5 (`--only-verified --fail`); gitleaks SARIF-upload via `codeql-action` (vereiste GHAS + security-events:write op private repo → "Resource not accessible by integration") → `actions/upload-artifact@v4`; `actions/checkout@v4→v5` (Node 24); permissions → least-privilege (`contents: read`). Download-URLs geverifieerd; dev-run groen bevestigd. Commit `1ed4d85`, gecascadeerd dev→test→main via worktree. **DNS-migratie OVH→Hetzner voltooid** (detail in 5.11.3): NS omgezet, alle subdomeinen + mail resolveren autoritatief via Hetzner. **`monitoring.holidaibutler.com` orphan-record VERWIJDERD** (redundant met status+grafana; had geen vhost/cert op prod). **UptimeKuma admin-pw reset** via DB-update — LES: bcrypt-hash NIET door een geneste shell halen (`$` in de hash corrumpeert via dubbele expansie); scriptbestand of hash-uit-file gebruiken. **Niet-technische managementsamenvatting** opgeleverd als Word-document (`HolidaiButler-Infrastructuurbewaking-4-lagen.docx`): 4 lagen + verantwoordelijkheid/escalatie-tabel + sectie "twee complementaire bewakingslagen" (infra-4-lagen vs applicatie-Agents healthMonitor/performanceWachter/anomaliedetective/financialMonitor). Geen prod-services geraakt; Frank's parallel-files (imageSelector/media*) ongemoeid. |
| **5.11.3** | **2026-06-13** | **Laag 4 externe uptime-monitor — LIVE (UpCloud Helsinki) + DNS-bevinding (INC-2026-06-11-001 follow-up)**. Sluit de 4-laagse defense-in-depth: Laag 1-3 (systemd Restart + OnFailure-Threema + cron health-check) draaien óp prod en falen mee bij hele-server-uitval. Laag 4 = externe monitor op **ander failure-domein**. **Provider-keuze onderbouwd (na GDPR/EU-onderzoek)**: **UptimeKuma self-hosted op UpCloud Helsinki** — Fins/EU-soeverein (geen US-moeder), 99,99% SLA op instap-tier (~€3,20/mnd), Helsinki = los stroomnet/provider t.o.v. Hetzner-Nürnberg. Afgewogen tegen UptimeRobot (EU-bedrijf maar US-dataopslag onder SCC's), BetterStack (EU-data maar Delaware Inc.), netcup (DE goedkoper maar Neurenberg-DC = zelfde stad als prod), Scaleway (FR duurder/Stardust te licht). Self-host gekozen voor maximale data-soevereiniteit (Frank's eis "100% GDPR/EU-proof"); geen end-user-PII door de monitor (data-minimalisatie). EU AI Act n.v.t. — ping-monitor is geen AI-systeem. **Deployment-kit gebouwd** in `deploy/uptime-kuma/`: `docker-compose.yml` (UptimeKuma + Caddy auto-TLS + threema-relay, intern netwerk, relay niet exposed), `Caddyfile` (TLS + security headers voor status.holidaibutler.com), `threema-relay/` (dependency-vrije Node-container die UptimeKuma JSON-webhook → Threema Gateway send_simple form-urlencoded vertaalt; rate-limit 5min + retry×2 + 10s timeout + structured logging + secret-masking, gelijk aan alert-threema.sh; `type:commonjs` package.json zodat require werkt ondanks repo-root ESM), `Dockerfile` (node:22-alpine non-root), `.env.example` + `.gitignore` (secrets nooit gecommit), `INSTALL.md` (provisioning + ufw/fail2ban/unattended-upgrades hardening + 6 monitors + Threema-koppeling + backup + E2E-checklist). **6 monitors**: holidaibutler.com / calpetrip.com / texelmaps.nl / api.holidaibutler.com/health / admin.holidaibutler.com / grafana.holidaibutler.com (60s interval). **SPOF-mitigatie wederzijdse monitoring**: prod's hb-health-check.sh krijgt een aparte reverse-probe naar de monitor (géén loopback --resolve, want remote) → valt de monitor-VPS weg, alarmeert prod via eigen Threema-pad. **Relay geverifieerd** (server-side, vóór deploy): credential-guard exit 1 + structured log; listen op 8099; /notify down+recovery → 202; /healthz → 200; dummy-creds → Threema HTTP 401 (pad werkt, géén echt bericht). **LIVE per 13-06-2026** op UpCloud Helsinki `212.147.237.156` (Ubuntu 26.04, Docker 29.5, storage-encryptie aan, ufw+fail2ban+unattended-upgrades). Host-hardening → Docker → kit-transfer → Threema-creds prod→monitor (zonder weergave, chmod 600) → `docker compose up`. **Config-as-code** via `setup_kuma.py` (uptime-kuma-api in wegwerp-container op het monitor-net): admin + webhook-notificatie naar relay + 6 monitors. **E2E volledig bewezen**: UptimeKuma-detectie→webhook→relay→echte Threema (test-monitor naar dood endpoint → relay `sent` msgId 86aabeb9; Frank bevestigde Threema-ontvangst). Dashboard publiek via Caddy auto-TLS op `https://status.holidaibutler.com` (Let's Encrypt tls-alpn-01 cert verkregen) + loopback `127.0.0.1:3001` voor SSH-tunnel-admin. **Wederzijdse monitoring live**: `hb-health-check.sh` op prod kreeg reverse-probe (IP gepind via --resolve, DNS-onafhankelijk) → bash -n OK, test-run schoon, laag4_monitor.state=OK. Alle 6 monitors UP. **BIJVANGST — DNS-bevinding (Laag 4 deed direct werk)**: api/admin/grafana/status + 7 andere subdomeinen resolveerden extern niet. Root cause: **half-afgemaakte OVH→Hetzner DNS-migratie** — zone-data staat compleet bij Hetzner maar NS-delegatie staat nog op OVH (`dns109/ns109.ovh.net`); OVH-zone mist de subdomeinen. Diensten zelf gezond (loopback 200/200/302). **Mail-gate ontdekt**: OVH-MX=OVH-mail vs Hetzner-MX=`www672.your-server.de`+MailerLite/MailerSend SPF+DMARC → NS-cutover kan e-mail breken; vereist bevestiging waar mailboxen staan. **Quick-fix uitgevoerd (optie 1)**: api/admin/grafana/status A-records in OVH-zone → direct bereikbaar. **NS-MIGRATIE OVH→Hetzner VOLTOOID 13-06** (Frank bevestigde mail op Hetzner): zone-audit (Hetzner=superset, mail+DKIM+DMARC compleet) → NS bij registrar (OVH) omgezet naar hydrogen/oxygen/helium.ns.hetzner → gepropageerd naar 1.1.1.1+8.8.8.8 → ALLE subdomeinen resolveren (ook de 7 eerder kapotte: sentry/monitoring/dashboard/dev/errors/admin.dev/admin.test), MX→Hetzner autoritatief. Rollback mogelijk (OVH-zone bestaat nog, ~1wk laten staan). **Follow-ups**: sentry extern hersteld (cert was geldig, was propagatie-transient); `monitoring.holidaibutler.com` = orphan DNS-record (geen vhost/cert op prod) → VERWIJDERD 17-06 (redundant met status+grafana; zie v5.11.4). **Les: na DNS-provider-migratie verifiëren dat NS-delegatie + álle subdomeinen + mail meegekomen zijn.** **Runbook** v1.1.0 §13 → ACTIEF. Geen prod-services geraakt behalve additieve reverse-probe in health-script (backup `/root/backups/hb-health-check.sh.pre-laag4-2026-06-13`); Frank's parallel-files (imageSelector/media*) ongemoeid. **Admin-dashboard-wachtwoord apart aan Frank verstrekt (te roteren).** |
| **5.11.2** | **2026-06-13** | **Apache hardening + Threema OnFailure-pipeline + cron health-check — INC-2026-06-11-001 prevention layer**. Root cause vorige outage (Apache 10u silent down 11-06 06:23-16:28 UTC): `apt-daily-upgrade` triggerde `systemd-reexec` tijdens unattended-upgrade → race op port 80 (`(98)Address already in use`) → Apache exit-code 1, niet ge-recoverd want base unit had `Restart=on-abort` (vangt alleen SIGABRT). **3-laagse defense-in-depth deployed**: **(1) Apache auto-recovery drop-in** `/etc/systemd/system/apache2.service.d/restart-on-failure.conf` met `Restart=on-failure` + `RestartSec=10s` + `StartLimitBurst=5` + `StartLimitIntervalSec=300` (StartLimit* expliciet in `[Unit]` section per systemd v230+ requirement, base had 10s window wat 5×10s retry niet toeliet). Verificatie via `systemctl show apache2`. **(2) Threema OnFailure-hooks** op 5 critical services: apache2 + pm2-root + redis-server + postgresql@16-main + mysql. Shell-script `/usr/local/bin/alert-threema.sh` doet directe POST naar Threema Gateway (`https://msgapi.threema.ch/send_simple` form-urlencoded) om single-point-of-failure in platform-core te vermijden — platform-core kan zelf down zijn wanneer we alert het meest nodig hebben. Credentials in `/etc/threema-alert.env` mode 0600 root-only (apart van platform-core/.env die www-data leesbaar moet zijn). Systemd template-unit `/etc/systemd/system/alert-threema@.service` met `EnvironmentFile`. Drop-in per service `[Unit] OnFailure=alert-threema@%n.service`. Rate-limit 5-min cooldown per service via lock-file `/run/alert-threema/<svc>.lock` voorkomt burst bij retry-loops. E2E test-trigger `systemctl start alert-threema@TEST-hardening-validatie.service` → HTTP 200, msg_id `fb71dba8407e2e49`, Frank bevestigde ontvangst op telefoon. **(3) Cron `*/2` external-perspective health-check** `/usr/local/bin/hb-health-check.sh` + `/etc/cron.d/hb-health` — curl --resolve `<host>:443:127.0.0.1` op holidaibutler.com + calpetrip.com + texelmaps.nl + api.holidaibutler.com/health. Bypassed lokale DNS, vangt 5xx + hang + vhost-misroute (systemd OnFailure ziet daar niets). Per-endpoint state-tracking in `/var/lib/hb-health/<host>.state` — 2-in-row failure vereist voor alert (filtert hiccups). Recovery-notificatie bij OK na FAIL. Initial run: 4/4 endpoints OK. **Side-incident**: DNS-migratie holidaibutler.com vanuit OVH parking IP 213.186.33.5 naar Hetzner 91.98.71.87 — public resolvers (8.8.8.8/1.1.1.1/9.9.9.9) updaten in minuten, maar lokale resolvers (router/ISP/NordVPN) hielden 1u vast door 3600s default-TTL bij OVH. Les: TTL verlagen naar 60-300s **24-48u vóór** elke DNS-migratie. **Niet gedekt** (backlog): externe uptime-monitor voor hele-server-uitval — vereist UptimeRobot/BetterStack op andere host + Frank-keuze provider. **Runbook**: `docs/runbooks/monitoring-and-alerting.md` (12 secties incl. nieuwe-service-toevoegen-procedure + troubleshooting matrix + backlog non-covered modes). **Backups**: `/root/backups/2026-06-11-monitoring-hardening/` met pre-install apache2 systemd state + alle nieuwe configs + onfailure.conf template. Commit `aac76f9` op dev (runbook 131 regels, 1 file). Cascade-merge naar test + main in afsluitende push. |
| **5.11.1** | **2026-06-11** | **INC-2026-06-10-005 SOCIAL_TOKEN_ENCRYPTION_KEY RESOLVED** — sluitende sessie van INC-2026-06-10 reeks (5/5 resolved). Twee encrypt-paden gefixed: (a) `SocialAccount.js:13` model `getEncryptionKey()` — fallback-chain `SOCIAL_TOKEN_ENCRYPTION_KEY || JWT_SECRET || 'default-key-change-me-32-chars!!'` gestript naar required env-var only + module-load fail-loud guard + request-time throw (fail-loud-zonder-fail-fast pattern voor request-path service, analoog INC-004); (b) `adminPortal.js:15950` Meta-connect inline cipher (zero-IV + `'default-key'` fallback) **DRY-by-replacement** → 8-regel inline cipher vervangen door 3-regel `SocialAccount.encryptToken()` aanroep. Lost latent Meta-decrypt-bug op — Calpe `id=1,2` Format-B-tokens waren niet decryptable door publisher (parsed alleen `:` Format-A) — nu Format A consistent. **Key generation**: `openssl rand -hex 32` → 32-byte random hex naar `platform-core/.env` (niet in chat geëxponeerd, audit SHA-256 hash `0d4166e7...b08b`). **Migration** `scripts/migrations/2026-06-11-inc005-reencrypt-social-tokens.mjs`: idempotent dual-format detect (`:` → A, else B), decrypt met OLD JWT_SECRET-derived key + correct IV per format, re-encrypt met NEW key + `crypto.randomBytes(16)`, dry-run vs commit mode, per-row try/catch. Dry-run: 5 access + 1 refresh decrypted, 0 failures. Commit: 5 + 1 written, alle Format A. Post-verify: alle 5 actieve `social_accounts` rows decryptable met nieuwe key (lengths 201/206/253 chars natural OAuth-range). PM2 restart `holidaibutler-api --update-env` (pid 2830701, uptime 24s, online), geen `SOCIAL_TOKEN env-var missing` warnings. **Backups**: `/root/backups/2026-06-11-inc005/` (.env pre-rotation + social_accounts.pre-inc005.json 7 rijen + 2 source backups). **Compliance**: GDPR Art 32 state-of-the-art (dedicated key + random-IV per record) + OAuth 2.0 RFC 6819 §5.1.4.5 encrypted-at-rest. **Documentatie**: `docs/security/incidents/2026-06-11-social-token-encryption-key-leak.md` (10 secties) + `SECURITY.md §10.2` INC-005 ⏳ → ✅. Sessie-vervolg: T1 shared costLogger.js (Temporal-worker context) + T2 brand-knowledge filter in trendAggregator + pre-commit hook validatie (in dezelfde sweep). |
| **5.11.0** | **2026-06-10** | **End-of-Day Security + Enterprise Sweep — 11 commits over 1 dag**. **Page Builder Weather block v6 (4b7fd3e voorganger commits)**: enterprise Validated RAG handler met Optie D Layer 3 NER grounding (5% hallucinatie-threshold platform-wide via `outputValidator.DEFAULT_HALLUCINATION_THRESHOLD` 0.10 → 0.05 — Frank instructie 'oudere doelgroep, hallucinatie = trigger om niet meer terug te keren') + auto-retry max 2 + EU AI Act provenance signatures + 10 weather-buckets met activity-guidance (thunderstorm→indoor, rainy→covered, snowy→winter, hot→shade/water, pleasant→outdoor, etc.) + multi-tenant geographic-relevance HARD RULE (tenant-agnostic via DB-cache `getOtherDestinationNames` + GEOGRAPHIC_EXCLUSIONS prompt-block + entity-filter) + slug-based query support. WeatherWidget hybrid server/client split met postMessage live-edit listener. Editor layout-aware preview (forecast chips conditional + 'Layout: Compact/Detailed' chip + validation badges per locale met hallucinationRate+retries tooltips). Data-cleanup: `brand_knowledge.id=7` (Texel-PDF mis-linked to Calpe destination_id=1) gedelete + backup. **Vier security-incidents geremedieerd op 1 dag** (pattern-niveau bewijs `process.env.X \|\| 'literal'` anti-pattern endemic): INC-2026-06-10-001 (OWM API key gelekt in `mobile/ProgramCard.tsx` client-bundle + WEATHER_COORDS hardcoded multi-tenant lock — rotated + stripped + proxy via `/api/v1/weather/public?slug=X`), INC-2026-06-10-002 (SISTRIX API key hardcoded fallback in `seoMeester/sistrixClient.js` — rotated + stripped + module-load fail-loud guard), INC-2026-06-10-003 (SimpleAnalytics API key hardcoded fallback ×2 in `trendspotter/websiteTrafficCollector.js` + `reisleider/index.js` + 3e overgeziene locatie in `adminPortal.js` regels 5651-5652 gevonden tijdens Group B triage — rotated + stripped + fail-loud guards; vendor SimpleAnalytics B.V. Tilburg NL bevestigd EU-soeverein i.p.v. initieel-foutief "SimilarWeb UK" gelabeld; Frank's correctie tijdens audit), INC-2026-06-10-004 (SESSION_SALT dev-fallback `'hb-salt'` actief productie-secret in `personaliseerder/index.js` — geroteerd via `openssl rand -hex 32` + graceful hash-degradation pattern). **INC-2026-06-10-005 candidate gedocumenteerd** (niet geremedieerd): `SOCIAL_TOKEN_ENCRYPTION_KEY` fallback-chain naar `'default-key'` voor AES-256-CBC OAuth-token encryption in `adminPortal.js:15950` — vereist OAuth re-encryption migration + Frank GO. **Secrets-tooling enterprise-grade**: gitleaks 8.28.0 + trufflehog 3.95.5 op `/usr/local/bin/`. Repo: `.gitleaks.toml` (default + 14 HB-specific rules incl. `hb-fallback-credential-pattern`) + `.gitleaks.baseline.json` (133→29 findings = 78% reductie over 4 commits zonder destructive history-rewrite) + `.pre-commit-config.yaml` + `.github/workflows/secret-scan.yml` (delta-mode CI). Trufflehog verified-only deep-scan over volledige history = 0 verified findings (alle historische keys revoked/expired). `docs/security/SECURITY.md` (12 secties: scope, secret-categorieën, implementatie-patronen A/B/C, tooling, incident-response, compliance-mapping GDPR/EU AI Act/ISO 27001/OWASP/CIS, baseline-strategie, vendor-verificatie best-practice, dev-placeholder fallbacks review-list, anti-patterns catalogue uit INC-001/002/003/004). Vier incident-rapporten (12-secties format) onder `docs/security/incidents/`. **Trendspotter + Reisleider agents geactiveerd** via Temporal worker: agentRegistry-registratie reeds aanwezig (regels 397, 445), nieuwe activities `agentActivities.js` (runTrendspotter/Reisleider voor Destination + multi-tenant gate `getActiveDestinationIdsForSA`), workflows `trendspotterWeeklyWorkflow.js` (cron `45 3 * * 0` Sunday 03:45 UTC, overlap=SKIP) + `reisleiderContextWorkflow.js` (on-demand), Temporal Schedule `trendspotter-weekly-v1` registered via `scripts/temporal/setupTrendspotterSchedule.js`. CostLog enum uitgebreid met `simpleanalytics` + `sistrix` (de laatste was used since INC-002 maar niet eerder geregistreerd). `websiteTrafficCollector` v2.2.0 met SHA-256 provenance signature in `trending_data.raw_data` JSON (EU AI Act Art 50) + cost-log helper. Manual trigger 2026-06-10 17:27-17:37 UTC: workflow ID `trendspotter-test-1781112442075`, 2/2 destinations succeeded, 0 failed, Calpe SA=1 trend met provenance `1c0ab60259f9`, Texel GT=1 (`wadden sea`). Documentatie `docs/agents/trendspotter-activation.md` (200+ regels). Known follow-up: MongoDB cost_logs buffer-timeout in Temporal-worker context (mongoose lazy-connect — non-blocking, doc'd). **Commits-chain** (origin/dev → origin/test → origin/main op end-of-day): `b25d9fb` (INC-001 OWM rotation) → `389ca7d` (Weather v6 feat + 5% threshold + multi-tenant) → `0eb0d97` (gitleaks tooling + SECURITY policy + INC-001 docs) → `881d2ea` (baseline group A legacy-archive allowlist -90) → `949a7db` (group C+D templates+workflows+tests -16) → `3858bd9` (group B docs-placeholder rule-tuning -24) → `dc3d4ee` (INC-002 SISTRIX rotation) → `66a3616` (INC-003 SimpleAnalytics rotation) → `4b7fd3e` (Trendspotter+Reisleider activation) → `e10b321` (INC-004 SESSION_SALT) → `8a34531` (INC-003 cleanup in adminPortal) → `cc6cb9d` (docs SECURITY §10 + §10.2 update). Test-merge `b94ade7`, main-merge `4699586`. 26 bestanden, ~6800 insertions / ~250 deletions. Vendor-policy: alle gebruikte AI/data-vendors EU/UK-adequacy compliant (Mistral Paris, DeepL Köln, OpenWeather London UK-adequacy, SimpleAnalytics Tilburg NL, SISTRIX Bonn DE, Pixtral via Mistral). Verboden anti-vendor: SimilarWeb (Israel HQ), Semrush (US), Quantcast. |
| **5.6.2** | **2026-05-18** | **FASE A Extra Blokken (A4+A5+A6) + parallel-sessie commits**. (A4) **Knowledge Base DOCX native rendering** — backend `GET /brand-sources/:id/preview` serveert DOCX nu inline (was: JSON content_text excerpt); frontend `mammoth^1.12.0` (lazy 491.72 kB chunk) converteert ArrayBuffer naar HTML, gerenderd in sandboxed iframe (`sandbox=""`, inline CSS, geen scripts, geen external loads). Smoke E2E: BUTE persbericht 103882-byte DOCX -> 5356 char HTML, 0 warnings. Vite 8 build 711ms. Commit `b80beb9`. (A5) **Temporal worker OTel + Sentry bootstrap** — `tracing.js` service.name nu env-driven (`process.env.OTEL_SERVICE_NAME || 'hb-platform-core'`, achterwaarts compatibel); `worker.js` importeert tracing als eerste import (ESM hoisting); PM2 `OTEL_SERVICE_NAME=hb-temporal-worker restart --update-env` + `pm2 save`. Tempo DELTA `['hb-platform-core']` -> `['hb-platform-core', 'hb-temporal-worker']`. Mem +130 MB normaal voor OTel instrumentations. Commit `4bc19a8`. (A6) **Grafana 11.3 UI + Tempo datasource** — Grafana container in bestaande Tempo Docker stack (`/opt/tempo/docker-compose.yml` extended met grafana service + persistence), auto-provisioned Tempo datasource via `/opt/grafana/provisioning/datasources/tempo.yaml` (uid=tempo, isDefault, http://tempo:3200, editable=false). 24-char random admin password in `/opt/grafana/.env` mode 600. Apache vhost `grafana.holidaibutler.com.conf` (HTTP, mod_proxy_wstunnel voor Grafana Live, pattern v5.2.0 sentry). `docs/infrastructure/GRAFANA-SETUP.md` met operations runbook, DNS-instructie, Let's Encrypt steps, SSH-tunnel pre-DNS toegang. Commit `40eedcc`. **Parallel-sessie commits** (Opus 4.6 terminal): `3b293c7` gateway content cache invalidation (adminPortal.js refactor 35716 lines), `342bcda`/`4cead47` test↔dev merges (impliciete branch-drift naar dev, MEMORY-regel pragmatisch geaccepteerd). Disjuncte file-sets met mijn werk, geen conflict. **Audit per blok 8/8 PASS**: health 3-5/5, Tempo trace-export werkend, datasource API returns, Apache 302 reverse-proxy redirect. **Pending Frank-acties (Blok A6)**: DNS A-record `grafana.holidaibutler.com -> 91.98.71.87` + `certbot --apache -d grafana.holidaibutler.com --redirect`. Tussentijds toegang `ssh -L 3000:127.0.0.1:3000 holidaibutler-prod`. **Rollback**: A4 git revert `b80beb9` + npm uninstall mammoth; A5 git revert `4bc19a8` + pm2 unset env; A6 `cp /opt/tempo/docker-compose.yml.bak.pre-grafana-20260518_0916 ...` + docker compose down grafana + a2dissite. 5 commits (b80beb9, 4bc19a8, 40eedcc + parallel 3b293c7, merge 4cead47), 5 files in code (brandSources.js, brandProfileService.js, MerkProfielSections.jsx, tracing.js, worker.js) + 1 doc (GRAFANA-SETUP.md) + 1 dependency (mammoth^1.12.0) + 3 server infra-files (docker-compose.yml, tempo.yaml datasource, apache vhost). |
| **5.8.0** | **2026-05-18** | **Notification Producers for Content Events**. NotificationService had infrastructuur (tabel, CRUD, frontend NotificationsCenter) maar slechts 2 producers (POI sync + tier promotion). 5 producers toegevoegd: publish success/failed (publisher/index.js), AI content generation complete (workers.js), content approved (adminPortal.js), publish-now failed (adminPortal.js). Alle non-blocking, target platform_admin. Notificaties verschijnen nu bij publicatie, AI generatie en goedkeuring. |
| **5.7.0** | **2026-05-18** | **Gateway Cache Invalidation + Auto-Crop + Content Studio Fixes**. (1) `invalidateContentCache()` gateway-helper op 13/25 mutatie-endpoints (was 5). Handmatig aangemaakte items verschijnen nu direct. (2) Auto-crop bij image-koppeling: `ensurePlatformCompatibleMedia()` met `PLATFORM_ASPECT_RULES` (IG 4:5-1.91:1, FB lenient, etc.). Sharp center-crop naar ideaal formaat, opgeslagen als variant met `parent_media_id` + `auto_platform`. Zero-effort voor gebruiker. DB: `media.parent_media_id` + `media.auto_platform` kolommen. (3) Concept-titel propagatie naar child items bij rename. (4) Tenant-cache invalidatie na concept update. |
