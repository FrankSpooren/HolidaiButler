...
### Open Punten (per 24-06-2026)

- **REJECTED-FSM** (open — te diagnosticeren/fixen): een afgewezen item kan niet opnieuw goedgekeurd worden; daarnaast bestaat "afgewezen-maar-gepland" (`approval_status='rejected'` mét `scheduled_at`, bv. item 303). `transitionStatus` cleart `scheduled_at` niet bij overgang naar `rejected` (wel bij `deleted` en `approved`-na-`scheduled`), en de kalender toont zulke items als gedateerd. Aparte opdracht.
- **FSM-drift-risico**: dubbele bron — `approvalStateMachine.js` (`TRANSITIONS`) + mirror `contentWorkflowMachine.js` (`DEFAULT_TRANSITIONS`). Helper `allowedTransitions()` (`approvalStateMachine.js:155`) leest de **niet-tenant DEFAULT**-map. De-dup als losse opschoning aanbevolen.
