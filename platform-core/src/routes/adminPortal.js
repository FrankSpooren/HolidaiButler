...
router.get('/dashboard', adminAuth('reviewer'), destinationScope, async (req, res) => {
  try {
    ...
    try {
      ...
      result.data.kpis.agents = { total: AGENT_METADATA.length, alerts: health?.alerts || 0, errors: health?.errors || 0, jobs: health?.jobs || 0 };
    } catch {
      result.data.kpis.agents = { total: AGENT_METADATA.length, alerts: 0, errors: 0, jobs: 0 };
    }
...
function calculateAgentStatus(lastRun, schedule, meta) {
  // Fase 1 fix: returns healthy | warning | error | deactivated (never unknown)
  if (meta?.active === false) return 'deactivated';

  if (!lastRun) {
    if (!schedule) return 'healthy';
    const p = String(schedule).split(' ');
    if (p[3] && p[3] !== '*') return 'healthy';
    if (p[4] && p[4] !== '*') return 'warning';
    if (p[2] && p[2] !== '*') return 'warning';
    return 'error';
  }
  ...
  try {
    ...
    const agents = AGENT_METADATA.map(meta => {
      ...
      return {
        ...
        status: meta.active === false ? 'deactivated' : 'warning',
        lastRun: null,
        destinations: meta.type === 'A' ? { calpe: { lastRun: null, status: 'warning' }, texel: { lastRun: null, status: 'warning' } } : null
      };
    });
...
router.get('/content/items/:id', adminAuth('editor'), async (req, res) => {
  try {
    ...
    if (item.destination_id) {
      try {
        const [[dest]] = await mysqlSequelize.query(
          'SELECT default_language, supported_languages FROM destinations WHERE id = :destId',
          { replacements: { destId: item.destination_id } }
        );
        if (dest) {
          let supportedLangs = [];
          try { supportedLangs = typeof dest.supported_languages === 'string' ? JSON.parse(dest.supported_languages) : (dest.supported_languages || []); } catch { /* empty */ }
          item.destination_config = {
            defaultLanguage: dest.default_language || 'en',
            supportedLanguages: supportedLangs,
          };
        }
      } catch { /* non-blocking — fallback to UI dropdown */ }
    }
...
router.patch('/content/items/:id', adminAuth('editor'), writeAccess(['platform_admin', 'destination_admin', 'poi_owner', 'content_manager', 'editor']), async (req, res) => {
  try {
    ...
    // Enforce single-language destinations: only accept body_<defaultLanguage>
    const langBodyFields = ['body_en', 'body_nl', 'body_de', 'body_es', 'body_fr'];
    try {
      const [[itemDest]] = await mysqlSequelize.query(
        `SELECT d.default_language, d.supported_languages FROM content_items ci
         JOIN destinations d ON d.id = ci.destination_id WHERE ci.id = :id`,
        { replacements: { id: Number(id) } }
      );
      if (itemDest) {
