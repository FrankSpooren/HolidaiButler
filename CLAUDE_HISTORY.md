...
### Frank feedback iteraties (tussendoor)
...
- Analytics/Analyse verwarring opgelost: één Analytics pagina met 4 tabs
- Content Rapport gescheiden van platform analytics (GET /content/report)

### Fase 4: Kwaliteit + Performance (Opdracht 14-15)
- **Opdracht 14**: Vite code-splitting (6 chunks: vendor-react 53KB, vendor-mui 806KB, vendor-charts 112KB, vendor-data 42KB, vendor-interactive 14KB, app 491KB gzip). Asset cleanup 1.3GB→7.9MB. Cache-Control headers (assets 1yr immutable, HTML no-cache). Sourcemaps disabled in productie.
- **Opdracht 15**: WCAG 2.1 AA audit (27 violations gevonden). 21 fixes: 16 aria-labels op icon-only buttons, 5 keyboard access (role=button + tabIndex + onKeyDown) op klikbare Cards/Boxes.

### Learnings
- **MUI sx numerieke waarden**:  in sx =  (theme spacing), niet . Altijd string  gebruiken voor absolute pixels
- **Vite minifier stript CSS uit template literals**: backtick strings met CSS selectors (#, .) worden leeggestript. Gebruik plain strings of inline styles
- **MUI Collapse neemt horizontale ruimte in**: zelfs bij  bepaalt de content width de parent breedte. Gebruik  om children rechts uit te lijnen
- **createPortal noodzakelijk voor fixed positioning**: MUI's flex layout in AdminLayout breekt . Portal naar document.body is de enige betrouwbare oplossing
- **sessionStorage overleeft F5**: gebruik in-memory React state voor session-only dismiss
- **Deploy stap essentieel**:  output in admin-module/dist/ moet gekopieerd worden naar /var/www/admin.holidaibutler.com/ (Apache DocumentRoot)
- **Diagnose eerst, code daarna**: de onboarding cirkel-positie kostte 10+ pogingen omdat er blind gefixt werd. Browser console diagnose (getBoundingClientRect) loste het in 1 stap op
