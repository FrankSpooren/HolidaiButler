...
## Sessie 11-05-2026: Content Studio Popup Consolidatie

### Context
Content Studio had twee verschillende content item popups:
- **ContentItemDialog** (904 LOC, inline in ContentStudioPage.jsx) — geopend via Acties kolom "Bewerken"
- **ConceptDialog** (1881 LOC, eigen bestand) — geopend via titelklik op tabelrij

### Diagnose
1. ContentItemDialog was ouder (Wave 5/6 periode), concept-gebaseerd model kwam later (v4.36.0)
2. ContentItemDialog had governance-features (Comments, Versies, Workflow, Approve/Reject, Share)
3. ConceptDialog had modernere features (AI Herschrijven, Alternatieven, Platform tabs, Validatie, Performance)
4. Beide bleven naast elkaar bestaan — nooit geconsolideerd

### Uitgevoerd
**Consolidatie:**
- ConceptDialog uitgebreid met 6 governance-features uit ContentItemDialog (+284 LOC)
- ContentItemDialog volledig verwijderd uit ContentStudioPage.jsx (-916 LOC)
- Acties-kolom "Bewerken" omgeleid naar ConceptDialog via setConceptDialogId

**4 Bug Fixes:**
1. SEO-drempel 80→70 + geen harde blokkade (Approve knop altijd beschikbaar, waarschuwing bij <70)
2. Titel opslaan: contentService.updateItem(concept.id) → contentService.updateConcept(concept.id)
3. Preview images: MUI sx objectFit specificity → native <img> met object-fit: contain
4. FB Approve geblokkeerd: gevolg van fix #1

### Bestanden
- `admin-module/src/components/content/ConceptDialog.jsx` (1881→2165 LOC)
- `admin-module/src/pages/ContentStudioPage.jsx` (4293→3377 LOC)
- `admin-module/src/components/content/PlatformPreview.jsx` (objectFit fix)

### Gearchiveerd Changelog (verplaatst uit CLAUDE.md bij v4.87.0)
