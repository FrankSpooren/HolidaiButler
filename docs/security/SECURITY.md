...
- ✅ `platform-core/src/routes/adminPortal.js:15950` + `platform-core/src/models/SocialAccount.js:13`: ~~`SOCIAL_TOKEN_ENCRYPTION_KEY || JWT_SECRET || 'default-key'` / `... || 'default-key-change-me-32-chars!!'`~~ — **GERESOLVEERD INC-2026-06-10-005** (sessie 2026-06-11): dedicated key gegenereerd (`openssl rand -hex 32`) + fallback-chains gestript + module-load fail-loud guard. `adminPortal.js:15950` inline zero-IV cipher **DRY-by-replacement** → `SocialAccount.encryptToken()` aanroep (random-IV, Format A consistency). Migration `scripts/migrations/2026-06-11-inc005-reencrypt-social-tokens.mjs` re-encrypted 5 access + 1 refresh tokens (3 Format A + 2 Format B → all Format A). Latent Meta-token decrypt-bug ook gefixed (Calpe id=1,2 waren niet decryptable door publisher). Zie `docs/security/incidents/2026-06-11-social-token-encryption-key-leak.md`.
