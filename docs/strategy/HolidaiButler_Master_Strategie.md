...
**Status**: FASE IV COMPLEET. FASE V IN PROGRESS. FASE VI IN PROGRESS. FASE VII IN PROGRESS (VII-A t/m VII-D COMPLEET). **Page Builder Enterprise + Admin UI Gap-Close COMPLEET (v4.66.0)**: 22 blokken enterprise-geüpgraded (ARIA, container queries, schema.org, srcset). 12 nieuwe admin UI componenten. BUTE taal-pipeline. tierPromotionAgent #26. 305 endpoints, adminPortal.js v3.50.0. 94 BullMQ jobs. 39 agents. CLAUDE.md v4.66.0. MS v8.20. **Fase 15 Foundation Stack COMPLEET (v4.74.0)**: NATS JetStream + Temporal + OTel + A2A + MCP. **Fase 16 A2A First-Light COMPLEET**: 3 cross-agent flows. CLAUDE.md v4.74.0. MS v8.23. **Fase 17 COMPLEET (v4.75.0)**: 71 inter-agent flows (46 A2A skills, 2 Temporal workflows). Dashboard fix materialized agent_status. CLAUDE.md v4.75.0. MS v8.24. **Fase 18 Inter-Agent Flow Completion COMPLEET (v4.76.0)**: 106 inter-agent flows (103 A2A skills, 3 Temporal workflows). 57 per-agent skill bestanden (Zod+OTel). 106 AsyncAPI 3.0 specs. De Promotor toegevoegd (38 A2A cards). CLAUDE.md v4.76.0. MS v8.25. **Media Library v2.0 Enterprise COMPLEET (v4.48.0)**: 20 opdrachten, 4 fasen (ML-1 t/m ML-4). media tabel 12 naar 40 kolommen. 4 nieuwe tabellen. 33 nieuwe endpoints (285 totaal). 15 frontend componenten (~2.500 LOC). Image editor (12 filters). 3 AI endpoints. GDPR compliance. i18n 5 talen. adminPortal.js v3.47.0. 66 BullMQ jobs. CLAUDE.md v4.48.0. MS v8.09. **Content Studio UX & Undo COMPLEET (v4.47.0)**: Sanitization consistentie (auto-fill was enige route zonder sanitizer). 7 enterprise tooltips op alle actiebuttons. Undo functionaliteit voor auto-fill, auto-schedule en campagne. Calendar edit button + concept koppeling. Publish performance records voor Analytics. Auto-fill concept+image koppeling. 252 admin endpoints. adminPortal.js v3.44.0. CLAUDE.md v4.47.0. MS v8.08. **Content Studio Multi-Tenant Fixes COMPLEET (v4.45.0)**: 12 fixes voor BUTE content_only destination — manual items, repurpose, image resolution, publisher multi-tenant, social account config, multi-tenant publishing, media picker. generateFromTitle() voor items zonder body. BUTECS System User token + igAccountId geconfigureerd. 252 admin endpoints. adminPortal.js v3.44.0. CLAUDE.md v4.45.0. MS v8.06. **Corporate Landing Page Upgrade COMPLEET (v4.44.0)**: Volledige redesign holidaibutler.com B2B pagina — 9 opdrachten. Hero "25 AI Agents. Eén Platform. Nul concessies." + demo modal (zakelijke e-mail validatie). 9 module cards met gecorrigeerde USP-teksten. Count-up stats balk (251/100+/35+/25/1). "Van Data, via Beleving tot Resultaat" procesbeschrijving met 3 geverifieerde KPI proof points (HubSpot/Statista/ETC). Live projecten showcase (CalpeTrip/TexelMaps/PubliQio). EU-First Technology Stack (6 providers met vlaggen). i18n 5 talen (NL/EN/DE/ES/FR) via extern i18n.js (162 keys). Mobiele UX: scroll-snap carousels, floating CTA, taal-dropdown met vlaggen, hamburger menu. Privacy pagina (11 secties GDPR) + PubliQio privacy geharmoniseerd. 252 admin endpoints. adminPortal.js v3.44.0. CLAUDE.md v4.44.0. MS v8.05. **Fase 19 Resilience/Closure/Cross-Domain COMPLEET (v4.77.0)**: 131 inter-agent flows, 7 Temporal workflows, 131 AsyncAPI specs. 37 healthy agents. CLAUDE.md v4.79.0. MS v8.28. **Fase 20.B-2 Discovery Restore + CI/CD Safety (v4.81.0)**: OSM-First Discovery Pipeline hersteld na rsync --delete dataverlies. 6 prospect endpoints + osmDiscoveryService.js + DiscoveryProspect model. CI/CD pre-deploy safety check voorkomt herhaling. 128 prospects intact. CLAUDE.md v4.81.0. MS v8.31. **Dashboard Acties & Snelkoppelingen (v4.82.0)**: Acties vereist dismiss/delegeer/gelezen-ongelezen + aanpasbare snelkoppelingen. 2 DB tabellen, 9 endpoints. 314 endpoints. adminPortal.js v3.51.0. CLAUDE.md v4.82.0. MS v8.30. **Fase B Multi-Tenant Scale COMPLEET (v4.94-v4.99)**: 7 work-blokken + post-fixes (18 commits). Blok 1 Workflow Sealing (FSM gateway 12 sites + provenanceAuditMonitor). Blok 2 Real-Time Frontend (TanStack Query mutations + Socket.IO realtime + per-tenant queryKey). Blok 3 FSM Productisering (XState v5 + workflow_configurations DB-driven + domainEventBus NATS-stijl + webhookDispatcher HMAC+retry). Blok 4 Delayed Publishing (BullMQ delayed-jobs per item, 60s precisie, orphan safety-net). Blok 5 Content Caching (Redis per-tenant + readReplica abstractie + SWR defaults). Blok 6 Reviewer UX (AIQualityPage Dashboard + SentenceCitations + CSV export). Blok 7 Provenance UI (ProvenancePanel + verify endpoint + pdfkit audit-PDF + real-time body watch + Merk Profiel deep-link). 47 legacy content_items backfilled met provenance signature. EU AI Act Article 50 compliance op item-niveau. 9 nieuwe backend services + 6 frontend componenten. workflow_configurations + webhook_endpoints + webhook_deliveries tables. BUTE 1-step + WarreWijzer 2-step beta workflows actief. 322 endpoints. CLAUDE.md v4.99.0. MS v8.40. **v5.9.0 Hetzner SSL Incident & Recovery COMPLEET (20-05-2026)**: 42u productie-outage hersteld met enterprise-grade CA-pinned SSL (Hetzner officiele cert, TLSv1.3) + defensive Sequelize pool (max 3, retry exp backoff) + mysql2 versie-consistency 3.15.3. Agenda module hersteld: agenda_events tabel + ETL 843 events + Event.js afterFind JSON-parse hook. admin-module/backend verwijderd (Scenario A). FASE B Sessie 1 pre-werk (nvm + Node 22.22.3 + ecosystem.config.cjs). 5 spawn-tasks afgerond, 4 openstaand. CLAUDE.md v5.9.0. MS v8.46. <!-- HETZNER-SSL-RECOVERY-V5.9.0 -->
...
| **8.28** | **05-05-2026** | **Content Studio Enterprise Fixes (v4.79.0)**: Taal-fixes single-lang destinations (body mapping BUTE/WarreWijzer/Alicante, LANGS filtering, body_language param). Schedule timezone (6 write-paths .toISOString()→lokale tijd). Publisher image lookup volgorde (media library EERST). Duplicate+Republish endpoints (direct publish, eigen concept). Kalender UX (published/failed buttons). 305 endpoints (+2). CLAUDE.md v4.79.0. MS v8.28. **Fase 20.B-2 Discovery Restore + CI/CD Safety (v4.81.0)**: OSM-First Discovery Pipeline hersteld na rsync --delete dataverlies. 6 prospect endpoints + osmDiscoveryService.js + DiscoveryProspect model. CI/CD pre-deploy safety check voorkomt herhaling. 128 prospects intact. CLAUDE.md v4.81.0. MS v8.29. **OSM Discovery Full Platform Integration (v4.82.0)**: 13 gaps gefixed — scrapeApproved enterprise pipeline (Apify->POI.create->Bronze/Silver/Gold->Classify->Reviews->Images), Dashboard pendingProspects, De Verkenner #39 agentRegistry, Worker OSM-first, i18n 5 talen. 2 pre-bestaande bugs (adminPortal SyntaxError, DashboardPage dismissedMenuAnchor). 6 commits, 16 bestanden. CLAUDE.md v4.82.0. MS v8.30. **Dashboard Acties & Snelkoppelingen (v4.82.0)**: Acties vereist dismiss/delegeer/gelezen-ongelezen + aanpasbare snelkoppelingen. 2 DB tabellen, 9 endpoints. 314 endpoints. adminPortal.js v3.51.0. CLAUDE.md v4.82.0. MS v8.30. |
...
| **8.40** | **15-05-2026** | **Fase B Multi-Tenant Scale COMPLEET (v4.94-v4.99)**. 7 work-blokken + post-fixes, 18 commits. Blok 1 Workflow Sealing: FSM gateway 12 sites, Stap 2 buttons FSM-driven, provenanceAuditMonitor 3-fail alert. Blok 2 Real-Time Frontend: TanStack Query useMutation optimistic, Socket.IO realtimeService per-tenant rooms, useRealtimeContent hook, ReactQueryDevtools. Blok 3 FSM Productisering: XState v5 contentWorkflowMachine, workflow_configurations DB-driven (BUTE 1-step + WW 2-step beta), domainEventBus NATS-stijl, webhookDispatcher HMAC+retry. Blok 4 Delayed Publishing: contentPublishScheduler BullMQ delayed-jobs (60s precisie), orphan safety-net cron, backfill 8 items. Blok 5 Caching: tenantCacheService Redis per-tenant, readReplicaSequelize abstractie, TanStack SWR defaults. Blok 6 Reviewer UX: AIQualityPage Dashboard met Recharts trend/top-entities/retry/CSV, SentenceCitations chip-first redesign. Blok 7 Provenance UI: ProvenancePanel real-time body watch (1500ms debounce auto re-verify), verify-provenance endpoint, pdfkit audit-PDF, Merk Profiel deep-link met scroll+pulse, 47 legacy items backfilled. 5/8 INSERT paden persisteren provenance (alle AI-flow). 9 nieuwe backend services, 6 frontend componenten, 3 nieuwe DB tabellen. 322 endpoints. adminPortal.js v3.25.0. CLAUDE.md v4.99.0. MS v8.40.** |
...
*Laatst bijgewerkt: 15 mei 2026 -- **Fase B Multi-Tenant Scale COMPLEET (v4.94-v4.99)**: 18 commits, 7 work-blokken. FSM gateway productisering (XState v5), real-time Socket.IO, BullMQ delayed-jobs precisie, Redis per-tenant cache, AI Quality Dashboard, EU AI Act Provenance UI met pdfkit audit-rapport + real-time tamper-detection + Merk Profiel deep-link. 9 nieuwe backend services, 6 frontend componenten, 3 nieuwe DB tabellen (workflow_configurations + webhook_endpoints + webhook_deliveries). BUTE 1-step + WarreWijzer 2-step beta workflows actief. 47 legacy content_items retro-actief provenance backfilled. 322 endpoints. CLAUDE.md v4.99.0. MS v8.40.*
