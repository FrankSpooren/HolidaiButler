...
class ContextResolver {
    static resolvePositionalReference(query, previousResults) {
    ...
    static resolveSemanticReference(query, previousResults) {
        const queryLower = query.toLowerCase();
        ...
        for (const pattern of semanticPatterns) {
            ...
            if (hasKeyword) {
                const foundPOI = previousResults.find(result => result.title.toLowerCase().includes(pattern.poi.toLowerCase()));
                if (foundPOI) {
                    return foundPOI;
                }
            }
    ...
    static resolveDirectPOIMention(query, previousResults) {
        const queryLower = query.toLowerCase();
