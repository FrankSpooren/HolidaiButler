...
export class MistralService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      // Log the request to Mistral
      flowLogger.logMistralRequest(text);
      logger.info(`🔮 Sending request to Mistral API: "${text}"`);
      ...
      // Log the response from Mistral
      flowLogger.logMistralResponse(embedding, timeMs);
      logger.info(`✅ Mistral API response received: ${embedding.length} dimensions in ${timeMs}ms`);
      logger.debug(`📊 First 5 embedding values: [${embedding.slice(0, 5).map(v => v.toFixed(4)).join(', ')}...]`);
      
      return embedding;
    } catch (error) {
      const timeMs = Date.now() - startTime;
      logger.error(`❌ Failed to generate embedding after ${timeMs}ms:`, error);
  ...
  async generateChatCompletion(messages: Array<{role: string, content: string}>): Promise<string> {
    ...
    try {
      logger.info(`💬 Sending chat completion request to Mistral API`);
      
      // Use the correct Mistral TypeScript SDK structure
      ...
      // Handle the content properly (it might be string or ContentChunk[])
      ...
      const timeMs = Date.now() - startTime;
      logger.info(`✅ Mistral chat completion received in ${timeMs}ms`);

      return contentString;
    } catch (error) {
      const timeMs = Date.now() - startTime;
      logger.error(`❌ Failed to generate chat completion after ${timeMs}ms:`, error);
      
      // Fallback to intelligent detection if Mistral API fails
      logger.info('🔄 Falling back to intelligent intent detection');
      const userMessage = messages.find(m => m.role === 'user')?.content || '';
      return this.intelligentIntentDetection(userMessage);
    }
  }

  private intelligentIntentDetection(query: string): string {
    const queryLower = query.toLowerCase();
    
    // More intelligent pattern matching for better intent recognition
    ...
    // Detect primary intent with better patterns
    if (queryLower.includes('when') || queryLower.includes('open') || queryLower.includes('closed') || queryLower.includes('hours')) {
      ...
      // Default to search_poi for general queries
    ...
    let confidence = 0.5;
    if (primaryIntent !== 'search_poi') confidence += 0.2;
    if (secondaryIntents.length > 0) confidence += 0.1;
    if (Object.values(intentContext).some(v => v === true)) confidence += 0.1;
  ...
  private generateSuggestedActions(primaryIntent: string, secondaryIntents: string[]): string[] {
  ...
  private simpleIntentDetection(query: string): string {
    const queryLower = query.toLowerCase();
