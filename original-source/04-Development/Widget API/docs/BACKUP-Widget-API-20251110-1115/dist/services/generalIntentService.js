...
exports.GeneralIntentService = void 0;
...
class GeneralIntentService {
    ...
    detectIntent(query) {
        ...
        logger_1.logger.info(`🧠 GENERAL INTENT DETECTION: "${query}"`);
        const detectedBoosts = [];
        ...
        for (const [category, intents] of Object.entries(this.intentPatterns)) {
            for (const [intentType, pattern] of Object.entries(intents)) {
                ...
                const confidence = matchScore > 0 ? 0.9 : 0;
                if (confidence > 0) {
                    const boost = {
                    ...
                    logger_1.logger.info(`   ✅ Detected: ${intentType} (confidence: ${confidence.toFixed(2)}, boost: ${pattern.boostFactor})`);
                }
            }
        }
        ...
        logger_1.logger.info(`🎯 INTENT RESULT: ${primaryIntent} (${detectedBoosts.length} boosts detected)`);
        return result;
    }
    generateSuggestedActions(boosts) {
        const actions = [];
    ...
    calculateIntentBoost(poi, intentResult) {
        if (intentResult.boosts.length === 0)
            return 0.5;
        let totalBoost = 0;
        let boostCount = 0;
...
exports.GeneralIntentService = GeneralIntentService;
//# sourceMappingURL=generalIntentService.js.map