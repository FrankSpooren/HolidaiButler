...
exports.INTENT_CONTEXT_CONFIG = void 0;
exports.generateIntentContextInterface = generateIntentContextInterface;
exports.detectContextFlags = detectContextFlags;
exports.INTENT_CONTEXT_CONFIG = {
    timeRelated: {
...
function generateIntentContextInterface() {
    const keys = Object.keys(exports.INTENT_CONTEXT_CONFIG);
    return `export interface IntentContext {
${keys.map(key => `  ${key}: boolean;`).join('\n')}
}`;
}
function detectContextFlags(query) {
    const queryLower = query.toLowerCase();
    const context = {};
    for (const [key, config] of Object.entries(exports.INTENT_CONTEXT_CONFIG)) {
