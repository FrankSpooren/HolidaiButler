/**
 * Onboarding Controller - ENTERPRISE LEVEL
 * ========================================
...
exports.saveStep = async (req, res, next) => {
  try {
    ...
    // Validate sequential onboarding
    const [user] = await query(
      'SELECT onboarding_step FROM Users WHERE id = ?',
      [userId]
    );

    if (stepNum > (user.onboarding_step || 0) + 1) {
      return res.status(400).json({
        success: false,
        error: {
          code: 'SEQUENTIAL_ONBOARDING_REQUIRED',
          message: `Please complete step ${user.onboarding_step + 1} first`
        }
      });
    }
    ...
    switch (stepNum) {
      ...
      case 4: {
        // Step 4: Optional preferences (dietary, accessibility)
      ...
      case 5: {
        // Step 5: Account creation (handled separately during signup)
        // This step just confirms everything is saved
        valid = true;
        break;
      }
...
exports.complete = async (req, res, next) => {
  try {
    ...
    if (user.onboarding_step < 4) { // Steps 1-4 must be completed (Step 5 is account creation)
      return res.status(400).json({
        success: false,
        error: {
          code: 'INCOMPLETE_ONBOARDING',
          message: `Please complete all onboarding steps. Current step: ${user.onboarding_step + 1}`
        }
      });
    }
