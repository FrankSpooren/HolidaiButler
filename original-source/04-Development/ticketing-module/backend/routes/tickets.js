...
router.get('/bookings/:bookingId', authenticate, async (req, res) => {
  try {
    ...
    // Verify user owns this booking (or is admin) - userId is now INTEGER
    if (booking.userId !== parseInt(req.user.id) && req.user.role !== 'admin') {
...
/**
 * POST /api/v1/tickets/bookings/:bookingId/cancel
 * Cancel a booking
 */
...
router.get('/:ticketId', authenticate, async (req, res) => {
  try {
    ...
    // POI model niet beschikbaar in standalone mode - skip include
    ...
    // Verify user owns this ticket (userId is now INTEGER)
    if (ticket.userId !== parseInt(req.user.id) && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        error: 'Access denied',
      });
...
router.post('/:ticketId/resend', authenticate, async (req, res) => {
  try {
    ...
    if (deliveryMethod === 'email' || !deliveryMethod) {
      // Property access changed: ticket.holder.email → ticket.holderEmail
